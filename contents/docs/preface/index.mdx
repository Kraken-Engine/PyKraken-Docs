...
## Navigating Classes
For nearly every class, you can write them in code as `kn.[ClassName]`. If a class cannot be accessed directly
from `kn`, the documentation will specify which submodule it belongs to, and you can write it as `kn.[submodule_name].[ClassName]`.
For example, the "Capsule" tab under the "Classes" section documents the `kn.Capsule` class, whereas the "MotorJoint" class
is to be accessed as `kn.physics.MotorJoint`.

Just below the class description, you'll often find a list of constructor signatures. These show you how to
instantiate the class with different parameters. For example, the "InputAction" class has five different constructors,
one of which being `InputAction(gamepad_button: GamepadButton, slot: int = 0)`.
